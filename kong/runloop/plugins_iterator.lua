...
local function load_plugin_configuration(ctx,
  ...
  if plugin ~= nil and plugin.enabled and
    (plugin.run_on == "all" or
    (ctx.is_service_mesh_request ~= true and plugin.run_on == "first") or
    (ctx.is_service_mesh_request == true and plugin.run_on == "second")) then
